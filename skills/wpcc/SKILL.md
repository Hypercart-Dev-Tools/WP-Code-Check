...
# Warn if the target doesn't look like a WP project — but always proceed
if ! find "$TARGET" -maxdepth 3 -name "*.php" -print -quit 2>/dev/null | grep -q .; then
  echo "Warning: no PHP files found under $TARGET — may not be a WordPress project."
  echo "  Proceeding anyway. If this is a JS-only or non-PHP project, results may be limited."
fi
...
```bash
# Check timeout first — a timed-out run may produce no/partial JSON
if [ "$EXIT_CODE" -eq 124 ]; then
  echo "Scanner timed out (300 s limit). To fix:"
  echo "  • Scope --paths to a subdirectory (e.g. /path/to/plugin/includes)"
  echo "  • Exclude large asset dirs: add them to EXCLUDE_DIRS in the scanner"
  exit 124
fi
...
# The HTML report is generated by json-to-html.py; if that helper is absent or failed,
# this will be empty — degrade gracefully to showing the JSON log path instead.
HTML_REPORT=$(grep -o '📊 HTML Report: .*' "$REPORT_JSON" 2>/dev/null | sed 's/📊 HTML Report: //' || echo "")
SCANNER_DIR=$(dirname "$(dirname "$(dirname "$SCANNER")")")
LATEST_JSON_LOG=$(ls -t "$SCANNER_DIR/dist/logs/"*.json 2>/dev/null | head -1 || echo "")
...
Full report: <HTML_REPORT if available, else LATEST_JSON_LOG, else "see scanner stdout">
```
