...
- A per-scan registry cache file generated from `dist/PATTERN-LIBRARY.json` is now consumed by `load_pattern()`, eliminating most per-pattern JSON parsing and Python subprocesses on the hot path.
...
**Phase 3 status note (2026-01-17):** A minimal in-memory loader is already partially in place via a per-scan registry cache generated by `dist/lib/pattern-loader.sh` from `dist/PATTERN-LIBRARY.json`. For v1.x, the remaining Phase 3 work is deliberately scoped to hardening this path (primarily staleness checks and clearer flags) rather than introducing additional moving parts. Performance measurement remains in Phase 4. JSON output has also been tightened so that `check-performance.sh --format json --no-log` skips automatic registry updates, guaranteeing a clean JSON-only contract for large scans (e.g. WooCommerce) and CI consumers. The fixture harness (`dist/tests/run-fixture-tests.sh`) now strips any non-JSON prefix before validating JSON, making tests resilient to environment-specific noise while still enforcing the single-document JSON contract.
