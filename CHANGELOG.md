...
- **Report formatting for files analyzed** – Added thousands separators to the
  “Files Analyzed” line in both the HTML generator (`dist/bin/json-to-html.py`)
  and the shell fallback (`dist/bin/json-to-html.sh`), and mirrored the same
  formatting in the inline HTML output path of `dist/bin/check-performance.sh`
  for consistent project info display.
