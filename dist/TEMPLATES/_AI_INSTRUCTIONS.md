...
2. Locate generated JSON: `dist/logs/[TIMESTAMP].json`
   - For scans generated with **WP Code Check 1.3.36 or later**, this file is a
     single valid JSON document.
   - For scans generated by **earlier versions**, the log may contain non-JSON
     lines (for example Python tracebacks) before the JSON payload. When analyzing
     an older log, strip everything before the first line that starts with `{` or
     prefer rerunning the scan on a newer version.
