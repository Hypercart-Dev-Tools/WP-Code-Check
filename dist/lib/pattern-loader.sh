...
load_pattern() {
  ...
  pattern_search=$(grep '"search_pattern"' "$pattern_file" | head -1 | sed 's/.*"search_pattern"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/')
  
  # Export for use in calling script
  export pattern_id pattern_enabled pattern_category pattern_severity pattern_title pattern_search
